...
static HANDSHAKE_HEADER: &str = "[HANDSHAKE]";
static POST_HEADER: &str = "[POST]";
static GETCOUNT_HEADER: &str = "[GET COUNT]";

impl MessageType {
    fn header(&self) -> &'static str {
        match self {
            MessageType::Handshake => HANDSHAKE_HEADER,
            MessageType::Post => POST_HEADER,
            MessageType::GetCount => GETCOUNT_HEADER,
        }
    }
}
...
impl Client {
    ...
    fn send(&mut self, addr: &str, msg: Message) -> CommsResult<Response> {
        let opt = self.connections.get_mut(addr);
        match opt {
            ...
            Some(conn) => match conn {
                ...
                Connection::Open(server) => {
                    let resp = server.receive(msg);
                    if let Err(CommsError::ServerLimitReached(_)) = resp {
                        self.connections
    ...
    fn is_open(&self, addr: &str) -> bool {
        let opt = self.connections.get(addr);
        match opt {
            Some(conn) => match conn {
    ...
    fn count_closed(&self) -> usize {
        self.connections
            .iter()
            .filter(|(_, v)| matches!(v, Connection::Closed))
            .count()
    }
}
        ...
        match msg.msg_type {
            ...
            MessageType::Post => {
                if self.post_count == self.limit {
                    return Err(CommsError::ServerLimitReached(self.name.clone()));
                }
                self.post_count += 1;
